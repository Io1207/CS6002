...
for t in range(1, T):
    ...
    true_skills_over_time[t] = np.clip(true_skills_over_time[t-1] @ transition_matrix + noise, 1, 3)


reported_skills_over_time = true_skills_over_time.copy()
reported_skills_over_time[:, 1, :]=np.clip(reported_skills_over_time[:, 1, :] -.5, 1, 3)
reported_skills_over_time[:, 0, :]=np.clip(reported_skills_over_time[:, 1, :] + .5, 1, 3)
